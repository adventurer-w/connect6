...
public class BoardMaster extends Board {
	...
	void findblanks(){
		for(int i = 0; i < SIDE*SIDE; i++){
			if(battle[i] > 0 && get(i)== EMPTY){
				changeRoads(i);
				int score = RoadTable.evaluateChessStatus(whoseMove(), roadTable);
				unchangeRoads(i);
				pointslist.add(new Node(i, score));
			}				
		}
	}
	/**寻找路上的空白点*/
	...
	/** 一 ， 生成必胜着法（我方已经四连或者五连)*/
	public MoveMaster findwinMoves(){
		...
		if(winroad != null){
		    ...
		    move = new MoveMaster(index0, index1);
		 }
		return move;
	}
	
	/** 二 ， 一般情况下的着法生成 寻找好点*/
	public  ArrayList<MoveMaster> findGenerateMoves() {
		ArrayList<MoveMaster> moves = new ArrayList<>();
		pointslist.clear();
		findblanks();
		pointslist.sort(Node.scoreComparator);
		//筛选点，减少搜索时间
		int choose_num = POINTSWIDTH < pointslist.size()/2 ? POINTSWIDTH:pointslist.size()/2;
		int index = 0;
		for(Node node : pointslist){
			for(int i = ++index; i < choose_num; i++){
				MoveMaster move = new MoveMaster(node.getPos(), pointslist.get(i).getPos());
				changeRoads(move);
				move.setScore(RoadTable.evaluateChessStatus(whoseMove(), roadTable));
				unchangeRoads(move);
				moves.add(move);
			}
			choose_num--;
	...
	ArrayList<MoveMaster> findDoubleThreats() {
		ArrayList<MoveMaster> movelist = new ArrayList<>();
		pointlist.clear();
		...
		for (int i = 0; i < len; i++) {
			for (int j = i + 1; j < len; j++) {
				...
				if (countAllThreats(whoseMove().opposite()) >= 2) {
					MoveMaster testmove = new MoveMaster(pointlist.get(i), pointlist.get(j));
	...
	ArrayList<MoveMaster> findSingleBlocks(){
		...
		for(Integer point:pointlist){
			for(Node points: pointslist){
				if(point != points.getPos()){
					MoveMaster move = new MoveMaster(point, points.getPos());
					changeRoads(move);
					move.setScore(RoadTable.evaluateChessStatus(whoseMove(), roadTable));
					unchangeRoads(move);
					movelist.add(move);
	...
	//两个子全部用来堵
	ArrayList<MoveMaster> findDoubleBlocks(){
		ArrayList<MoveMaster> movelist = new ArrayList<>();
		pointlist.clear();
		for(int i = 0; i <SIDE*SIDE;i++) vis[i] = false;
		//对方连四或者连五
		...
		for (int i = 0; i < pointlist.size(); i++)
		{
			for (int j = i + 1; j < pointlist.size(); j++)
			{
				...
				//能破除
				if (four.size() + five.size() == 0){
					MoveMaster move = new MoveMaster(pointlist.get(i), pointlist.get(j));
					move.setScore(RoadTable.evaluateChessStatus(whoseMove(), roadTable));
					movelist.add(move);
				}										
				unchangeRoads(pointlist.get(i));
	...
	ArrayList<MoveMaster> findTripleBlocks(){
		ArrayList<MoveMaster> movelist = new ArrayList<>();
		pointlist.clear();
		for(int i = 0; i <SIDE*SIDE;i++) vis[i] = false;
		//对方连四或者连五
		...
		for (int i = 0; i < pointlist.size(); i++)
		{
			for (int j = i + 1; j < pointlist.size(); j++)
			{
				...
				if (nowthreats < minthreats){
					...
					movelist.add(new MoveMaster(pointlist.get(i),pointlist.get(j)));
				}
